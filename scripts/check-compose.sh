...
# local 개발을 위해 필요한 db(mariadb)와 minio가 실행중인지 확인하고, 실행중이지 않다면 docker-compose로 실행시키는 script입니다.
...
if [ "$MARIADB_RUNNING" -eq 1 ] && [ "$MINIO_RUNNING" -eq 1 ]; then
    echo "✅ All services are already running:"
elif [ "$MARIADB_RUNNING" -eq 1 ] || [ "$MINIO_RUNNING" -eq 1 ]; then
