...
import { coursesApi, CourseOffering, CoursesQuery } from '@/lib/api/courses'
...
const SEARCH_TYPE_OPTIONS = [
  ...
  { value: 'instructor', label: '담당교원' },
]

export default function CoursesOfferingPage() {
  ...
  const fetchCourses = async () => {
    try {
      setLoading(true)
      const response = await coursesApi.getOfferings(query)
  ...
  const handleYearSemesterChange = (value: string) => {
    ...
    if (value) {
      ...
      setQuery(prev => ({ ...prev, year: parseInt(year), semester, page: 1 }))
    } else {
      setQuery(prev => ({
        ...prev,
        year: undefined,
    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-2">
          <Link to="/admin/courses/offering/bulk-upload">
            <Button variant="info" radius="md" size="md">
              일괄 업로드
            </Button>
          </Link>
          <Link to="/admin/courses/offering/create">
            <Button variant="info" radius="md" size="md">
              개별 등록
            </Button>
          </Link>
                    ...
                    <tr key={course.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 font-body-14-medium text-gray-600">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            to={`/admin/courses/offering/edit/${course.id}`}
