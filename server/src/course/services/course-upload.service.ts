...
export class CourseUploadService {
  ...
  async uploadFromExcel(
    ...
    try {
      ...
      for (let i = 0; i < jsonData.length; i++) {
        ...
        try {
          ...
          if (!courseData.code || !courseData.name) {
            result.errors.push(`${rowIndex}행: 학수강좌번호와 교과목명은 필수입니다`);
            result.errorCount++;
            continue;
          }
      ...
      throw new BadRequestException(`Excel 파일 처리 중 오류: ${error.message}`);
    }
  }
  ...
  private parseRowData(row: any, uploadDto: CourseUploadDto): Partial<Course> {
    return {
      ...
      department: this.getFieldValue(row, ['개설학과', '학과', 'department']) || '',
      grade: this.parseGrade(this.getFieldValue(row, ['대상학년', '학년', 'grade'])),
      credit: this.parseCredit(this.getFieldValue(row, ['학점', 'credit'])),
      time: this.parseTime(this.getFieldValue(row, ['요일/교시', '시간', 'time'])),
      instructor: this.getFieldValue(row, ['담당교원', '교수', 'instructor']),
      classroom: this.parseClassroom(this.getFieldValue(row, ['강의실', 'classroom'])),
  ...
  async getExcelTemplate(): Promise<Buffer> {
    const templateData = [
      ['학수강좌번호', '교과목명', '교과목영문명', '개설학과', '대상학년', '학점', '요일/교시', '담당교원', '강의실', '강의유형'],
      ['IOT101-01', 'IoT 기초', 'IoT Fundamentals', '지능IoT학과', '1학년', 3.0, '월 09:00-12:00', '김교수', 'A101', '이론'],
      ['IOT201-01', '데이터구조', 'Data Structure', '지능IoT학과', '2학년', 3.0, '화 13:00-16:00', '이교수', 'A102', '이론'],
      ['IOT301-01', 'IoT시스템설계', 'IoT System Design', '지능IoT학과', '3학년', 3.0, '수 09:00-12:00', '박교수', 'A103', '실습']
    ];
